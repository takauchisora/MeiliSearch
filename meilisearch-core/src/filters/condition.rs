...
fn get_field_value<'a>(schema: &Schema, pair: Pair<'a, Rule>) -> Result<(FieldId, ConditionValue<'a>), Error> {
    ...
    let field = schema.id(key.as_str()).ok_or_else(|| {
...
fn compare_numbers(lhs: &Number, rhs: &Number) -> Option<Ordering> {
    match (lhs.as_i64(), lhs.as_u64(), lhs.as_f64(), rhs.as_i64(), rhs.as_u64(), rhs.as_f64()) {
...
impl<'a> Condition<'a> {
    pub fn less(item: Pair<'a, Rule>, schema: &'a Schema) -> Result<Self, Error> {
    ...
    pub fn greater(item: Pair<'a, Rule>, schema: &'a Schema) -> Result<Self, Error> {
        let (field, value) = get_field_value(schema, item)?;
    ...
    pub fn neq(item: Pair<'a, Rule>, schema: &'a Schema) -> Result<Self, Error> {
        let (field, value) = get_field_value(schema, item)?;
    ...
    pub fn geq(item: Pair<'a, Rule>, schema: &'a Schema) -> Result<Self, Error> {
        let (field, value) = get_field_value(schema, item)?;
    ...
    pub fn leq(item: Pair<'a, Rule>, schema: &'a Schema) -> Result<Self, Error> {
        let (field, value) = get_field_value(schema, item)?;
    ...
    pub fn eq(item: Pair<'a, Rule>, schema: &'a Schema) -> Result<Self, Error> {
        let (field, value) = get_field_value(schema, item)?;
