...
impl fmt::Display for SerializerError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            SerializerError::DocumentIdNotFound => f.write_str("Primary key is missing."),
            SerializerError::InvalidDocumentIdFormat => f.write_str(
                "a document primary key can be of type integer or string only composed of alphanumeric characters, \
                 hyphens (-) and underscores (_).",
            ),
            SerializerError::Zlmdb(e) => write!(f, "heed related error: {}", e),
            SerializerError::SerdeJson(e) => write!(f, "serde json error: {}", e),
            SerializerError::ParseNumber(e) => write!(f, "error while trying to parse a number: {}", e),
            SerializerError::Schema(e) => write!(f, "impossible to update schema: {}", e),
            SerializerError::UnserializableType { type_name } => write!(f, "{} is not a serializable type", type_name),
            SerializerError::UnindexableType { type_name } => write!(f, "{} is not an indexable type", type_name),
