...
use heed::{types::Str, BytesDecode, BytesEncode, RoRange, RoTxn, RwTxn};
...
impl<'a> BytesEncode<'a> for FacetData {
    ...
    fn bytes_encode(item: &'a Self::EItem) -> Option<Cow<'a, [u8]>> {
        ...
        let size = mem::size_of::<u64>() + first_size + item.1.len() * mem::size_of::<DocumentId>();
...
impl Facets {
    ...
    pub fn put_facet_document_ids(
        &self,
        writer: &mut RwTxn<MainT>,
        facet_key: FacetKey,
        doc_ids: &Set<DocumentId>,
        facet_value: &str,
    ) -> MResult<()> {
        Ok(self.facets.put(writer, &facet_key, &(facet_value, doc_ids))?)
    }

    pub fn field_document_ids<'txn>(
        &self,
        reader: &'txn RoTxn<MainT>,
        field_id: FieldId,
    ) -> MResult<RoRange<'txn, FacetKey, FacetData>> {
        Ok(self.facets.prefix_iter(reader, &FacetKey::new(field_id, String::new()))?)
    }

    pub fn facet_document_ids<'txn>(
        &self,
        reader: &'txn RoTxn<MainT>,
        facet_key: &FacetKey,
    ) -> MResult<Option<(&'txn str, Cow<'txn, Set<DocumentId>>)>> {
        Ok(self.facets.get(reader, &facet_key)?)
    }

    /// updates the facets  store, revmoving the documents from the facets
    /// provided in the `facet_map` argument
    pub fn remove(
        &self,
        writer: &mut RwTxn<MainT>,
        facet_map: HashMap<FacetKey, (String, Vec<DocumentId>)>,
    ) -> MResult<()> {
        for (key, (name, document_ids)) in facet_map {
            if let Some((_, old)) = self.facets.get(writer, &key)? {
                ...
                let new = sdset::duo::OpBuilder::new(old.as_ref(), to_remove.as_set())
                    .difference()
                    .into_set_buf();
